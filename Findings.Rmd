...
df %>% 
  ...
  select(id, "Team" = which_team_was_primarily_responsible_for_this_session, "Session Name" = what_was_the_name_of_the_session, "Location" = where_was_the_session_delivered, "Locatioon Group" = location_is_library, "Category" = how_would_you_categorise_this_session, "Content" = content) %>% 
  DT::datatable(
...
df %>% 
  mutate(across(where(is.character) & !what_was_the_name_of_the_session, as.factor)) %>% 
  select(id, "Team" = which_team_was_primarily_responsible_for_this_session, "Session Name" = what_was_the_name_of_the_session, "Delivery agent" = who_delivered_the_session, "Co-delivery agents" = with_whom_was_the_session_co_delivered, "Delivery Group" = delivery_group, "Category" = how_would_you_categorise_this_session, "Content" = content) %>% 
  DT::datatable(
...
df %>% 
  filter(was_the_session_designed_to_specifically_benefit_maori_or_pasifika == "Yes") %>% 
  mutate(across(where(is.character) & !what_was_the_name_of_the_session, as.factor)) %>% 
  select(id, "Team" = which_team_was_primarily_responsible_for_this_session, "Session Name" = what_was_the_name_of_the_session, "Māori, Pasifika, or both?" = which_community_was_the_session_designed_to_specifically_benefit, "Category" = how_would_you_categorise_this_session, "Content" = content) %>% 
  DT::datatable(
...
df %>% 
  ...
  mutate(across(where(is.character) & !what_was_the_name_of_the_session, as.factor)) %>% 
  select(id, "Team" = which_team_was_primarily_responsible_for_this_session, "Session Name" = what_was_the_name_of_the_session, "Communities" = other_communities_tidied, "Communities Other?" = communities_other, "Category" = how_would_you_categorise_this_session, "Content" = content) %>% 
  DT::datatable(
...
df %>% 
  mutate(what_was_the_target_age_group_for_the_session = case_when(
    is.na(what_was_the_target_age_group_for_the_session) ~ "None",
    !is.na(what_was_the_target_age_group_for_the_session) ~ what_was_the_target_age_group_for_the_session
  ), across(where(is.character) & !what_was_the_name_of_the_session, as.factor)) %>% 
  select(id, "Team" = which_team_was_primarily_responsible_for_this_session, "Session Name" = what_was_the_name_of_the_session, "Age group" = what_was_the_target_age_group_for_the_session, "Category" = how_would_you_categorise_this_session, "Content" = content) %>% 
  DT::datatable(
...
df %>% 
  filter(!is.na(how_would_you_categorise_this_session)) %>% 
  mutate(how_would_you_categorise_this_session = case_when(
    is.na(please_categorise_this_session_using_your_own_words) ~ how_would_you_categorise_this_session,
    !is.na(please_categorise_this_session_using_your_own_words) ~ please_categorise_this_session_using_your_own_words
  )) %>% 
  mutate(across(where(is.character) & !what_was_the_name_of_the_session, as.factor)) %>% 
  select(id, "Team" = which_team_was_primarily_responsible_for_this_session, "Session Name" = what_was_the_name_of_the_session, "Category Other?" = category_other, "Category" = how_would_you_categorise_this_session, "Content Other?" = content_other, "Content" = content) %>% 
  DT::datatable(
...
df %>% 
  ...
  select(id, "Team" = which_team_was_primarily_responsible_for_this_session, "Session Name" = what_was_the_name_of_the_session, "Location" = location, "Location group" = location_is_library) %>% 
  DT::datatable(
