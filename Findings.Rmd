...
df %>% 
  filter(was_the_session_designed_to_specifically_benefit_other_communities == "Yes") %>% 
  mutate(across(where(is.character) & !what_was_the_name_of_the_session, as.factor)) %>% 
  select(id, "Session Name" = what_was_the_name_of_the_session, "Other communities" = which_community_or_communities_was_the_session_designed_to_specifically_benefit, "Category" = how_would_you_categorise_this_session, "Content" = content) %>% 
  DT::datatable(
...
What was the primary focus or content of this session? Show only the content options that were selected for more than one session.
...
df %>% 
  filter(!is.na(how_would_you_categorise_this_session)) %>% 
  mutate(across(where(is.character) & !what_was_the_name_of_the_session, as.factor)) %>% 
  select(id, "Session Name" = what_was_the_name_of_the_session, "Category" = how_would_you_categorise_this_session, "Content" = content) %>% 
  DT::datatable(
...
df %>% 
  ...
  select(id, "Session Name" = what_was_the_name_of_the_session, "Location" = location) %>% 
  DT::datatable(
